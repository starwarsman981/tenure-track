...
const State = {
    ...
    initNewGame: function(name, typeKey, discKey, playerName) {
        ...
        for(let i=0; i<targetCount; i++) {
            ...
            for(let s=0; s<studentCount; s++) {
                ...
                // If a starting student is a G1, initialize their rotation properties
                if (newStudent.year === 1) {
                    newStudent.subfield = prof.field;
                    newStudent.rotationStatus = "Searching";
                    newStudent.rotationCount = 1;
                    newStudent.currentRotationLabId = prof.id;
                    newStudent.advisorId = null;
                    newStudent.funding = "TA";
                } else {
  ...
  advanceDay: function() {
    // --- NEW: PROCESS TIMED EVENTS ---
        ...
        // 2. CHECK EVENTS
        if(this.data.month === 4 && this.data.day === 1) {
            // Fires Graduation and Commencement notices on May 1st
    ...
    triggerDiscovery: function(prof) {
        ...
        // Re-balanced: Pushed thresholds higher so top-tier papers are true milestones
        const roll = (prof.hIndex / 4) + (Math.random() * 15); 
        let jObj = journals[5]; 
        if(roll > 55) jObj = journals[0]; // Nature
        else if(roll > 48) jObj = journals[1]; // Science
        else if(roll > 32) jObj = journals[2]; // JACS
        else if(roll > 24) jObj = journals[3]; // Angewandte
        else if(roll > 12) jObj = journals[4]; // Chem. Sci.
        ...
        // 5. Grant Supplement (Tuned downward to stop excessive cash accumulation)
        const supplementChance = Math.max(0.01, Math.min(0.12, realImpact * 0.012));
        let bonusHtml = "";
        
        if (Math.random() < supplementChance) {
    const variableCap = 30000 * (realImpact / 10);
    const bonusAmount = 10000 + Math.floor(Math.random() * variableCap);
    prof.funds += bonusAmount;

    bonusHtml = `
        <div style="background:#e8f8f5; border-left:4px solid #27ae60; padding:10px; margin:10px 0;">
            Grant supplement received: +$${bonusAmount.toLocaleString()}
        </div>
    `;
}
        // Save Record
...
processGrantCycle: function() {
    ...
    makeFacultyOffer: function(id, offerAmount) {
        ...
        if(Math.random() < chance) {
            ...
            // Avoids creating redundant "Dr. Dr." text headers
            newProf.name = c.name.startsWith("Dr. ") ? c.name : "Dr. " + c.name;
    ...
    processAcademicYearRollover: function() {
        ...
        // 2. PROMOTION - Disabled in May. Will handle in August during cohort intake.
        /*
        ...
        */
        ...
        // 3. INTAKE (Accepted Applicants -> G1 Rotations)
        const newCohort = this.data.admissions.pool.filter(a => a.status === 'Accepted');
        
        if(newCohort.length > 0) {
            newCohort.forEach(a => {
                // Find eligible faculty who match the incoming student's subfield choice
                let fieldFaculty = this.data.faculty.filter(f => f.field === a.interest);
                if (fieldFaculty.length === 0) fieldFaculty = this.data.faculty; // Safety fallback
                
                // Assign a starting lab for their very first 8-week section rotation block
                let startingRotationPI = fieldFaculty[Math.floor(Math.random() * fieldFaculty.length)];
                ...
                const newStudent = {
                    ...
                    year: 1,
                    subfield: a.interest,
                    rotationStatus: "Searching", // Tracked state: "Searching", "Matched", or "Unmatched"
                    rotationCount: 1,            // Tracks progression up to 6 total rotation blocks
                    currentRotationLabId: startingRotationPI.id,
                    advisorId: null,             // Set to a value only when a permanent match succeeds
                    funding: "TA",               // All G1 students remain locked on TA line funds
                    stats: a.stats,
                    pubs: 0
                };
            ...
            log.push(`✨ New Cohort: ${newCohort.length} G1s entered the lab rotation system.`);
    ...
    loadGame: function(input) { const load = (json) => { this.data = JSON.parse(json); UI.toggleGameView(true); UI.updateTopBar(this.data); Game.navigate('office'); }; if(input && input.files[0]) { const r = new FileReader(); r.onload = (e) => load(e.target.result); r.readAsText(input.files[0]); } else if (localStorage.getItem('tenureTrackSave')) load(localStorage.getItem('tenureTrackSave')); }, // <-- FIXED: Added comma here to safely link the next function

    // Iterates through all active G1 student records every 8 weeks to attempt mini-matches or swap labs
    advanceRotationWeeks: function() {
        let matchedCount = 0;
        let swapCount = 0;
        let failedCycleCount = 0;
        let reportDetails = [];

        // Filter out students who are currently active G1s and still actively rotating
        let rotatingStudents = this.data.students.filter(s => s.year === 1 && s.rotationStatus === "Searching");

        rotatingStudents.forEach(student => {
            let currentPI = this.data.faculty.find(f => f.id === student.currentRotationLabId);
            if (!currentPI) return;

            // Compute hard space availability cap rule: 35% of current lab group headcount, brand new hires default to 3
            let currentCount = this.data.students.filter(s => s.advisorId === currentPI.id).length;
            let spaceLimit = currentCount === 0 ? 3 : Math.ceil(currentCount * 0.35);
            let absoluteAllocatedCount = this.data.students.filter(s => s.advisorId === currentPI.id && s.year === 1 && s.rotationStatus === "Matched").length;
            
            let hasPhysicalSpace = (absoluteAllocatedCount < spaceLimit);
            
            // Standard general check probability target (~40% chance to match)
            let roll = Math.random() * 100;
            let successfulMatchRoll = roll < 40;

            if (successfulMatchRoll && hasPhysicalSpace) {
                // Settle student permanently into this lab
                student.rotationStatus = "Matched";
                student.advisorId = currentPI.id;
                currentPI.students.push(student.id);
                matchedCount++;
                reportDetails.push(`✅ ${student.name.split(' (')[0]} permanently joined the ${currentPI.name.split(' ').pop()} Lab (${student.subfield})`);
            } else {
                // Match fails or space was full. Advance their block index or set them to fallback track if maxed out (6 rotations)
                if (student.rotationCount >= 6) {
                    student.rotationStatus = "Unmatched";
                    failedCycleCount++;
                    reportDetails.push(`❌ ${student.name.split(' (')[0]} completed all 6 rotations without a permanent match. Staying on general TA line.`);
                } else {
                    // Collect alternative rotation labs matching their exact subfield domain
                    let fieldFaculty = this.data.faculty.filter(f => f.field === student.subfield && f.id !== currentPI.id);
                    if (fieldFaculty.length === 0) fieldFaculty = this.data.faculty; // Fallback context if single PI subfield exists
                    
                    let nextPI = fieldFaculty[Math.floor(Math.random() * fieldFaculty.length)];
                    student.currentRotationLabId = nextPI.id;
        ...
        // Recalculate financial burn strings across all modified faculty profiles
        this.data.faculty.forEach(f => this.recalcFacultyFinances(f));

        // Issue a notification email summarizing the rotation results directly to your inbox
        if (matchedCount > 0 || swapCount > 0 || failedCycleCount > 0) {
            let reportSummaryHtml = reportDetails.length > 0 ? `<div style="font-size:0.9rem; line-height:1.5; background:#fff; padding:12px; border:1px solid #ddd; margin-top:10px;">${reportDetails.join('<br>')}</div>` : '';
            this.addEmail(
                "Graduate Committee", 
                `Rotation Section Update`, 
                `<p>The 8-week lab rotation block has concluded.</p>
                 <ul>
                    <li>Students Successfully Matched: <strong>${matchedCount}</strong></li>
                    <li>Students Swapped to Next Lab: <strong>${swapCount}</strong></li>
                    <li>Students Unmatched (Cycle Exhausted): <strong>${failedCycleCount}</strong></li>
                 </ul>
                 ${reportSummaryHtml}`, 
                "notification"
            );
        }
    } // <-- FIXED: Removed formatting artifacts
}; // <-- FIXED: Kept the closing bracket of the State object here!
        ...
        for(let i=0; i<count; i++) {
            ...
            pool.push({
                ...
                background: {
                    ...
                    monthsExp: monthsExp,
                    prevRole: prevRole,
                    sopScore: Math.floor(sopScore)
                },

                stats: { 
                    brains: Math.floor(brains), 
...
const FacultyGenerator = {
    ...
    recalcFacultyFinances: function(prof) {
        ...
        myStudents.forEach(s => {
            ...
            if(s.funding === "RA") {
                // CHANGED: Fixed scope pointer to utilize main State configurations
                const stip = (State.COSTS.RA_WEEKLY * pol.stipendMod);
                weeklyBurn += stip;
                raCost += stip;
                raCount++;
            } else if (s.funding === "TA") {
        ...
        let parts = [];
        // CHANGED: Redirected 'this' lookup path straight to State object instance
        parts.push(`Lab Maintenance: $${State.COSTS.LAB_BASE}`);

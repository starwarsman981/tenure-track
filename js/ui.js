...
const UI = {
    ...
    renderFinance: function(data, tab) {
        ...
        const tabs = [{ id: 'overview', label: 'Overview' }, { id: 'budget', label: 'Budget & Ledger' }, { id: 'grants', label: 'Grant Watch' }];
        let tabHtml = `<div style="display:flex; gap:10px; margin-bottom:20px; border-bottom:1px solid #ddd; padding-bottom:10px;">`;
        tabs.forEach(t => { const active = t.id === tab ? 'background:#2c3e50; color:white;' : 'background:#eee; color:#555;'; tabHtml += `<button onclick="Game.setFinanceTab('${t.id}')" style="padding:8px 16px; border:none; border-radius:4px; cursor:pointer; font-family:inherit; ${active}">${t.label}</button>`; });
        tabHtml += `</div>`;
    ...
    renderFinanceOverview: function(container, data) {
        ...
        container.innerHTML = `
            <div style="display:flex; gap:15px; margin-bottom:20px;">
                <div style="background:white; border:1px solid #ddd; padding:15px; border-radius:4px; flex:1; text-align:center; border-left:4px solid ${FINANCE.COLORS.balance}"><div style="font-size:0.8rem; color:#888;">CASH BALANCE</div><div style="font-size:1.8rem; font-weight:bold;">$${data.budget.toLocaleString()}</div></div>
                <div style="background:white; border:1px solid #ddd; padding:15px; border-radius:4px; flex:1; text-align:center; border-left:4px solid ${netColor}"><div style="font-size:0.8rem; color:#888;">LAST WEEK NET</div><div style="font-size:1.8rem; font-weight:bold; color:${netColor}">${last.net >= 0 ? '+' : ''}$${last.net.toLocaleString()}</div></div>
                <div style="background:white; border:1px solid #ddd; padding:15px; border-radius:4px; flex:1; text-align:center; border-left:4px solid #f39c12"><div style="font-size:0.8rem; color:#888;">WEEKLY OVERHEAD</div><div style="font-size:1.8rem; font-weight:bold;">$${(det.idc || 0).toLocaleString()}</div></div>
            </div>
    ...
    renderFinanceBudget: function(container, data) {
        ...
        fin.weeklyLog.forEach(log => {
            ...
            ledgerHtml += `<div style="padding:8px 10px; border-bottom:1px solid #f9f9f9; display:flex; justify-content:space-between;"><div style="flex:1;"><strong>${log.date}</strong> <span style="color:#666;">${log.details}</span></div><div style="font-weight:bold; color:${logColor};">${log.net >= 0 ? '+' : ''}$${log.net.toLocaleString()}</div></div>`;
        });
        ledgerHtml += `</div>`;
        container.innerHTML = `<div style="background:#f9f9f9; padding:20px; border-radius:4px; margin-bottom:20px; text-align:center; border:1px solid #ddd;"><h3>Projected Year-End Result</h3><div style="font-size:2.5rem; font-weight:bold; color:${statusColor}">${fiscal.net >= 0 ? '+' : ''}$${Math.round(fiscal.net).toLocaleString()}</div><div style="color:#666;">Based on current weekly net + expected tuition drops</div></div><h3>Transaction Ledger</h3>${ledgerHtml}`;
    },

    renderFinanceGrants: function(container, data) {
        let html = `<table style="width:100%; border-collapse:collapse; font-size:0.9rem;"><thead style="background:#eee; text-align:left;"><tr><th style="padding:10px;">PI</th><th style="padding:10px;">Grant Name</th><th style="padding:10px;">Remaining</th><th style="padding:10px;">Runway</th></tr></thead><tbody>`;
        data.faculty.forEach(prof => {
            if(prof.grants.length === 0) { } else {
                prof.grants.forEach(g => {
                    const runway = prof.burnRate > 0 ? (g.remaining / prof.burnRate).toFixed(1) : "Inf";
                    let color = "black"; if(parseFloat(runway) < 6) color = "red";
                    html += `<tr style="border-bottom:1px solid #eee;"><td style="padding:10px;">${prof.name}</td><td style="padding:10px;">${g.name}</td><td style="padding:10px;">$${g.remaining.toLocaleString()}</td><td style="padding:10px; color:${color}; font-weight:bold;">${runway} mo</td></tr>`;
                });
            }
        });
        html += `</tbody></table>`;
